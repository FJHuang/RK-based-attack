...
def graph_incv3(x, y, i, x_max, x_min, grad):
    ...
    u4 = x + alpha * tf.sign(k3)
    with slim.arg_scope(inception.inception_v3_arg_scope()):
        logits, end_points = inception.inception_v3(u4, num_classes=FLAGS.num_classes, is_training=False)
    #logits = (end_points['Logits'])
    cross_entropy = tf.losses.softmax_cross_entropy(y,logits,label_smoothing=0.0,weights=1.0)    
    k4 = tf.gradients(cross_entropy, u4)[0] 
       
    noise = 1/6 * (k1 + 2 * k2 + 2 * k3 + k4)
...
def main(input_dir, output_dir):
    ...
    with tf.Graph().as_default():
        ...
        with slim.arg_scope(inception.inception_v3_arg_scope()):
            logits, end_points_1 = inception.inception_v3(x_input, num_classes=FLAGS.num_classes, is_training=False)
